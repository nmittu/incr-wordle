...
module Make (R : GameRules) = struct
  ...
  type t = 
    ...
    let enter_word t w =
      let target = R.get_new_target t.rule_data ~locked_in:(get_locked w t.target) ~old_target:t.target in
      if validate_word t w
      then
        if String.( = ) w t.target
        then { t with guesses = R.get_hints t.rule_data ~target:t.target ~word:w :: t.guesses }
        else { t with start = t.start; guesses = R.get_hints t.rule_data ~target:t.target ~word:w :: t.guesses; target }
