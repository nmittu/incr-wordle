...
let get_default () =
  ...
  let loaded =
    Option.value
      ~default:todays_game
      (LocalStorage.load_model "model" ~deserializer:(Converters.convert normal_mode_of_sexp))
  in
  if NormalGame.rule_data loaded.game = Game.NormalRule.get_today () then
    init (Normal loaded)
  ...
  let enter_word t =
    let helper (type a) (m : (module Game.GameEngine with type t = a)) (g:a) ~create_game_mode =
      let module M = (val m) in
      let g = if M.validate_word g t.input
      then (
        let g = M.enter_word g (String.lowercase t.input) in
        g)
      else g in
      {game = create_game_mode g; input = ""} in
    match t.game with
    | Normal gm -> 
        helper (module NormalGame) gm.game
